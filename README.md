...
Чтобы вывести карточку документа со скачиванием, используйте токен `[[document ...]]` на отдельной строке.
...
- `src` — путь к файлу в `public/...`. Обязательный параметр.
- `title` — заголовок карточки. Если не указан, берётся имя файла без расширения.
- `meta` — вторая строка карточки, например `Документ · XLSX`.
- `download` — имя файла при скачивании. Если не указано, используется `title`.
- `href` и `file` — альтернативы для `src`, если удобнее такой нейминг.

Поведение:

- Карточка открывает скачивание по клику.
- Название документа показывается в одну строку с `...`, если оно слишком длинное.
- Формат файла можно показывать отдельно через `meta`, а в `title` расширение не писать.
